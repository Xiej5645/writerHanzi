...
(function ($) {
    ...
    $.chineseInput = function (el, options) {
        ...
        self.init = function () {
            ...
            self.$toolbar = $('<div class="toolBar" id="chinese-toolbar-' + self.id + '"></div>').draggable();
            self.$active = $('<label class="chinese-checkbox" for="check_' + self.id + '" onclick="event.stopPropagation()"><input type="checkbox"' + (self.options.active ? ' checked="checked"' : '') + ' id="check_' + self.id + '"/> Chinese-input</label>');
            self.$input = $('<label class="chinese-checkbox" for="type_' + self.id + '" onclick="event.stopPropagation()"><input type="checkbox"' + (self.options.input.initial == 'traditional' ? ' checked="checked"' : '') + ' id="type_' + self.id + '"/> traditional</label>')
            self.$copy = $('<button id="copyButton" onclick="copyT(event)">Copy</button>');

            self.$toolbar.insertAfter(self.$el);
            self.$toolbar.css({ 'position': 'absolute', 'top': '125px', 'left': '110px', 'height': 35, 'z-index': 1000 }).show();
            ...
            } self.$copy.appendTo(self.$toolbar);
                ...
                if (options && options.length) {
                    ...
                    if (!$box.size()) {
                        $box = $(document.createElement('div')).draggable().
                            attr({ 'id': 'chinese-ime', 'class': 'ime-box' }).
                            html(self.html)
                        $('body').append($box);
                    }
